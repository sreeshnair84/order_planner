            ...
            if isinstance(sku_items, list):
                ...
                for item in sku_items:
                    if isinstance(item, dict):
                        ...
                        logging.warning(f"SKU item is not a dictionary, skipping: {type(item)} - {item}")
                return validated_items
            else:
                logging.warning(f"SKU items is not a list: {type(sku_items)} - {sku_items}")
                return []
        ...
        {{
            "sku_items": [
                {{
                    ...
                    "processing_remarks": "List any missing required details like SKU code, pricing, category, etc."
                }}
            ]
        }}

        If pricing, weight, or other details are not available, use null values and note the missing information in processing_remarks.
        Ensure quantity_ordered is always a positive integer.
        Always return a valid JSON object with the "sku_items" array.
        For processing_remarks, note any missing critical information such as: missing SKU code, missing pricing, missing category, unclear product description, etc.
        """
        ...
        for item in sku_items:
            try:
                ...
                if not isinstance(item, dict):
                    logging.error(f"SKU item is not a dictionary: {type(item)} - {item}")
                    continue
                
                # Log the item for debugging
                logging.debug(f"Processing SKU item: {item}")
                ...
                cur.execute(insert_query, (
                    ...
                    item.get('processing_remarks'),  # New field for processing remarks
                ))
            except Exception as item_error:
                logging.error(f"Failed to insert individual SKU item {item}: {item_error}")
        ...
        if row and row[4]:  # file_path exists
            ...
            try:
                ...
                if parsed_data.get("file_type") != "Unsupported":
                    ...
                    logging.info(f"Attempting to insert {len(sku_items)} SKU items for order {order_id}")
                    logging.debug(f"SKU items data: {sku_items}")
